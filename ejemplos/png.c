                ...
                int *width, int *height)
{
        ...
        *data = (unsigned char *)row_pointers;
        ...
        for (i = 0; i < *height; i++)
                row_pointers[i] = (png_byte*) malloc(png_get_rowbytes(png,
info));
...
int get_png_files(const char *path, char ***png_list) {
  ...
  while ((entry = readdir(dp)) != NULL) {
    ...
    if (npngs > 0 && npngs % 10 == 0)
      png_array =
          (char **)realloc(png_array, sizeof(char *) * 10 * (npngs / 10 + 1));
  }
 ...
 */
void LoadPng(FILE *file, unsigned char **data, char **clipData,
  ...
  png_read_info(png, info);
  png_get_IHDR(png, info, (png_uint_32 *)width, (png_uint_32 *)height, &depth,
...
int main(int argc, char **argv) {
  ...
  win = XCreateSimpleWindow(dpy, RootWindow(dpy, screen), 0, 0, WINDOW_WIDTH,
                            WINDOW_HEIGHT, 5, BlackPixel(dpy, screen),
  ...
  while (!stop) {
    ...
    ximage =
        XCreateImage(dpy, DefaultVisual(dpy, screen), DefaultDepth(dpy, screen),
                     ZPixmap, 0, (char *)data, width, height, 32, png_bytes);
    // if (event.type == KeyPress &&
    //     XwcLookupString(&event.xkey, text, sizeof(text), &key, 0, ) == 1) {
    //   if (text[0] == 'q')
    //     stop = 1;
    // }
